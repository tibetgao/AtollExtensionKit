...
final class AtollQuotaPresenter {
    ...
    private func dashboardHTML() -> String {
        ...
        *{box-sizing:border-box}html,body{margin:0;width:100%;height:236px;overflow:hidden;background:transparent;color:var(--ink);font-family:ui-monospace,"SFMono-Regular",Menlo,monospace;font-size:calc(10px*var(--scale));-webkit-font-smoothing:antialiased}button{font:inherit;color:inherit}.shell{height:236px;background:linear-gradient(180deg,rgba(15,14,18,.98),var(--panel));overflow:hidden}.top{height:31px;display:flex;align-items:center;gap:12px;padding:0 9px;border-bottom:1px solid var(--line)}
        ...
        <script>
        const bridge='http://127.0.0.1:\(CodexInteractionServer.port.rawValue)',token='\(bridgeCredentials.token)',body=document.getElementById('body');let shown=0,total=0,loading=false,historyOpen=false,lastMode='',expandedSessionID=null,lastCompletionID=null;
        ...
        function api(p){return bridge+p+(p.includes('?')?'&':'?')+'token='+encodeURIComponent(token)}function go(p){fetch(api(p),{mode:'no-cors'}).catch(()=>{})}function openQuota(){go('/open/quota')}function openThread(id){go('/open/thread?id='+encodeURIComponent(id))}function age(ts){let s=Math.max(0,Date.now()/1000-ts),v;if(s<60)return'now';if(s<3600)v=Math.floor(s/60)+'m';else if(s<86400)v=Math.floor(s/3600)+'h';else if(s<604800)v=Math.floor(s/86400)+'d';else if(s<2592000)v=Math.floor(s/604800)+'w';else if(s<31536000)v=Math.floor(s/2592000)+'mo';else v=Math.floor(s/31536000)+'y';return v+' ago'}function reset(ts){let s=ts-Date.now()/1000;if(s>0&&s<86400){let h=Math.floor(s/3600),m=Math.floor((s%3600)/60);return'↻ '+(h?h+'h ':'')+m+'m'}return'↻ '+new Date(ts*1000).toLocaleDateString(undefined,{weekday:'short',hour:'2-digit',minute:'2-digit'})}function niceModel(x){return(x||'Codex').replace(/^gpt-/,'').replaceAll('-',' ')}
